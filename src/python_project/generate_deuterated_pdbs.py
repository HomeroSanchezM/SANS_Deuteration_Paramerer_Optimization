...
Final results stored in:
    <prefix>_final_results/
	 - <prefix>_best_fitness.cvs  # header + one data row
	 - pdb/
	     - <prefix>_best_fitness.pdb
	     - ref/
                 - <protein>_total_deuteration.pdb
                 - <protein>_total_protonation.pdb
        - sans_simulation/
             - <protein>_best_fitness.dat
             - <protein>_best_fitness.log
	     - ref/
		  - <protein>_total_deuteration.dat
                  - <protein>_total_protonation.dat
		  - <protein>_total_deuteration.log
                  - <protein>_total_protonation.log
...
def create_final_result_folder(
    ...
    """
    Assemble a self-contained result folder for the best solution found.


    Args:
        best_chrom:        The best chromosome of the final generation.
        pdb_stem:          Stem of the source PDB file used as naming prefix
                           (e.g. ``"myprotein"`` produces ``myprotein_final_result/``).
        output_dir:        Working directory that contains the PDB files and
                           the ``ref/`` sub-directory.
        primus_dir:        Pepsi-SANS output directory that contains ``.dat``
                           files and its own ``ref/`` sub-directory.
        best_summary_path: Path to the per-generation ``best_fitness_summary.csv``

    Returns:
        ``Path`` of the created ``<pdb_stem>_final_result`` directory.
    """
    # Derive primus_out directory (same logic as the batch script)
    folder_name = output_dir.name
    prefix = folder_name.replace("_deuterated_pdbs", "")
    final_dir = str(output_dir.parent / f"{prefix}_final_results")
    logger.info(f"  final results in  : {final_dir}")
    ...
    pdb_ref_dir = Path(pdb_ref_dir)
    sans_dir = Path(sans_dir)
    sans_ref_dir = Path(sans_ref_dir)
    ...
    # ------------------------------------------------------------------
    # 1. Single-row CSV  (header + best chromosome)
    # ------------------------------------------------------------------
    ...
    with open(csv_dest, "w", encoding="utf-8") as dst_fh:
        dst_fh.write(lines[0])   # header
        dst_fh.write(lines[-1])  # best row
    ...
    best_pdb_dst = pdb_dir / f"{base_name}.pdb"
    if best_pdb_src.exists():
        shutil.copy2(best_pdb_src, best_pdb_dst)
        logger.info(f"  Best PDB copied         : {best_pdb_dst.name}")
    else:
        logger.warning(f"  Best PDB NOT found      : {best_pdb_src.name}")

    # ------------------------------------------------------------------
    # 3. Reference PDBs  (original filenames preserved)
    # ------------------------------------------------------------------
    ref_pdbs = sorted((output_dir / "ref").glob("*.pdb"))
    if not ref_pdbs:
        logger.warning("  No reference PDB files found in output_dir/ref/")
    for src in ref_pdbs:
        dst = pdb_ref_dir / src.name
        shutil.copy2(src, dst)
        logger.info(f"  Ref PDB copied              : {dst.name}")

    # ------------------------------------------------------------------
    # 4. Best SANS outputs (.dat, .log, .out)
    # ------------------------------------------------------------------
    ...
    for ext in (".dat", ".log", ".out"):
        ...
        if src.exists():
            dst = sans_dir / f"{base_name}{ext}"
            shutil.copy2(src, dst)
            copied_sans.append(dst.name)
            logger.info(f"  Best SANS {ext:5s} copied   : {dst.name}")
    if not copied_sans:
        logger.warning(
            f"  No SANS output found for stem '{best_dat_stem}' in {primus_path}"
        )

    # ------------------------------------------------------------------
    # 5. Reference SANS files  (original filenames preserved)
    # ------------------------------------------------------------------
    ref_sans_dir = primus_path / "ref"
    if ref_sans_dir.is_dir():
        ref_dats = sorted(ref_sans_dir.glob("*.dat"))
        if not ref_dats:
            logger.warning("  No reference .dat files found in primus_dir/ref/")
        for dat_src in ref_dats:
            dst_dat = sans_ref_dir / dat_src.name
            shutil.copy2(dat_src, dst_dat)
            logger.info(f"  Ref SANS .dat copied        : {dst_dat.name}")
            # .log (same stem)
            log_src = dat_src.with_suffix(".log")
            if log_src.exists():
                dst_log = sans_ref_dir / log_src.name
                shutil.copy2(log_src, dst_log)
                logger.info(f"  Ref SANS .log copied        : {dst_log.name}")
            # .out (same stem, fallback)
            out_src = dat_src.with_suffix(".out")
            if out_src.exists():
                dst_out = sans_ref_dir / out_src.name
                shutil.copy2(out_src, dst_out)
                logger.info(f"  Ref SANS .out copied        : {dst_out.name}")
    else:
        logger.warning(f"  Reference SANS directory not found : {ref_sans_dir}")
...
def main():
    ...
    for gen in range(1, cfg['generations']):
        ...
        # ------------------------------------------------------------------
        # 2. Identify tier-1 files to keep; remove all others
        # ------------------------------------------------------------------
        logger.debug(
        ...
        # ------------------------------------------------------------------
        # 3. Generate PDB files for tier-2/3 only
        # -----------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 4. Run Pepsi-SANS only on tier-2/3 (incremental mode)
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 5. Evaluate fitness for the full new population
